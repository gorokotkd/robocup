package team_b.module.algorithm;
...
public class FireClustering extends DynamicClustering {
  ...
  public Clustering calc() {
    System.out.println("Cluster 123 : " + clusterList.size());
    for (EntityID changed : worldInfo.getChanged().getChangedEntities()) {
      StandardEntity changedEntity = worldInfo.getEntity(changed);
      if (changedEntity.getStandardURN().equals(StandardEntityURN.BUILDING)) { // changedEntity(cE)
                                                                               // is
                                                                               // a
                                                                               // building
        Building changedBuilding = (Building) changedEntity;
        if (this.getClusterIndex(changedEntity) < 0) { // cE is not contained in
                                                       // cluster
          if (isBurning(changedBuilding)) { // cE is burning building
            ArrayList<
                EntityID> hostClusterPropertyEntityIDs = new ArrayList<>();

            // search host cluster
            for (List<StandardEntity> cluster : this.clusterList) {
              for (StandardEntity entity : cluster) {
                if (worldInfo.getDistance(entity,
                    changedBuilding) <= groupingDistance) {
            ...
            if (hostClusterPropertyEntityIDs.size() == 0) { // there is not host
                                                            // cluster : form
                                                            // new cluster
              List<StandardEntity> cluster = new ArrayList<>();
              clusterList.add(cluster);
              cluster.add(changedBuilding);
            } else if (hostClusterPropertyEntityIDs.size() == 1) { // there is
                                                                   // one host
                                                                   // cluster :
                                                                   // add
                                                                   // building
                                                                   ...
                                                                   // cluster
              int hostIndex = this
                  .getClusterIndex(hostClusterPropertyEntityIDs.get(0));
              clusterList.get(hostIndex).add(changedBuilding);
            } else { // there are multiple host clusters : add building to the
                     // cluster & combine clusters
              int hostIndex = this
                  .getClusterIndex(hostClusterPropertyEntityIDs.get(0));
              List<StandardEntity> hostCluster = clusterList.get(hostIndex);
              hostCluster.add(changedBuilding);
              for (int index = 1; index < hostClusterPropertyEntityIDs
                  .size(); index++) {
                int tergetClusterIndex = this
                    .getClusterIndex(hostClusterPropertyEntityIDs.get(index));
          ...
          if (!(isBurning(changedBuilding))) { // cE is not burning building
            ...
            List<
                StandardEntity> hostCluster = clusterList.get(hostClusterIndex);
              ...
              while (!(relatedBuilding.isEmpty())) {
                ...
                while (!(openedBuilding.isEmpty())) {
                  for (StandardEntity entity : relatedBuilding) {
                    if (worldInfo.getDistance(openedBuilding.get(0),
                        entity) <= groupingDistance) {
  ...
  public Clustering updateInfo(MessageManager messageManager) {
    ...
    //this.debugStdOut("Cluster 123 : " + clusterList.size());
    System.out.println("Cluster 123 : " + clusterList.size());
