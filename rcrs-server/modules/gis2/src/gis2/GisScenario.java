...
public class GisScenario implements rescuecore2.scenario.Scenario, CollapseSimCompatibaleScenarioV1_1 {
  ...
  private static final int DEFAULT_CIVILIAN_BURIEDNESS = 0;
  private static final int DEFAULT_CIVILIAN_HP = 2000;
  private static final int DEFAULT_STAMINA = 2000;
  private static final int DEFAULT_DAMAGE = 500;
  private static final int DEFAULT_BURIEDNESS = 0;
  private static final String DEFAULT_TEAM_NAME = "No Team";
  ...
  private void setupAgent(Human h, EntityID position, StandardWorldModel model, Config config)
    ...
    if (h instanceof Civilian) {
      h.setHP(DEFAULT_CIVILIAN_HP + 500);
    } else {
      h.setHP(DEFAULT_HP + 500);
    }
    //h.setHP(DEFAULT_HP);
    h.setDamage(DEFAULT_DAMAGE);
    if (h instanceof Civilian) {
      h.setBuriedness(DEFAULT_CIVILIAN_BURIEDNESS);
    } else {
      h.setBuriedness(DEFAULT_BURIEDNESS);
    }
    //h.setBuriedness(DEFAULT_BURIEDNESS);
    h.setDirection(0);
    ...
    h.setTeam(DEFAULT_TEAM_NAME);
    model.addEntity(h);
    System.out.println("Created Instance" + h.toJson());
  }
